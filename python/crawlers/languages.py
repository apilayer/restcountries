...
def __extractLanguagesData(html):
    ...
    for i, row in enumerate(table):
    	if i > 0:
    		...
    		language['twoLetterCode'] = columns[4].text.encode('utf-8')
    		language['threeLetterCode'] = columns[5].text.encode('utf-8')
    ...
    for countryJSON in countriesJSON:
        for languageJSON in countryJSON['languages']:
            for language in languages:
                if languageJSON['twoLetterCode'] == language['twoLetterCode']:
                    languageJSON['threeLetterCode'] = language['threeLetterCode']
